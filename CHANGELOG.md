...
- New **Control tower** dashboard page: every live session, across every channel (Telegram, API, widget, this dashboard), on one screen - channel, agent, model, turn count, and whether it's running a turn right now, filterable by agent/channel/search, with a one-click jump into the conversation and a `Stop` for one stuck mid-turn. `Session.status/1` now also reports `running`.
