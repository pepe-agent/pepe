...
### Fixed
- **A board card genuinely still being worked, past its board's `claim_timeout_s`, had no way to say so and could get force-blocked as stalled anyway** - `comment` (the closest existing action) never touched `claimed_at`, so nothing reset the stall clock for a long-running claim. New `board heartbeat` action (and `mix pepe board card heartbeat ID [--as NAME]`) resets it without changing status - a liveness signal, not progress logging.
...
- **A Telegram file over 20MB (Telegram's own bot download cap) silently vanished instead of being reported** - sent alone, it got a generic "couldn't download" message that didn't say why; sent together with other files in one message, it was dropped with no mention at all, so 2 files sent together could quietly turn into "only 1 arrived." Now names the actual reason (too big for a bot to fetch this way) for a single file, and tells you specifically which/how many were skipped when only part of a multi-file upload made it through.

### Added
- **`fetch_url` now returns a page's actual readable text by default, not the raw HTML around it** - nav bars, cookie banners, footers, and ad markup used to come back verbatim alongside the real content, burning context on none of it being the answer. New `Pepe.Readable` (built on `Floki`, not the `readability` hex package, which unconditionally pulls in httpoison/hackney for a URL-fetching helper this never needed and that conflicts with this project's own locked dependencies) drops known-noise elements and prefers a page's own `article`/`main`/`[role=main]` container, falling back to scoring the remaining blocks by how much of their text isn't inside a link. Pass `raw: true` to skip extraction and get the response body untouched (an API response, source code, a page you need the literal markup of); a page with nothing extractable falls back to the raw body automatically either way, and only a `text/html` response is ever touched at all.
- **New `memory_search` tool: an agent can search its own `MEMORY.md`/`USER.md`/`people.md` for one thing instead of reading the whole file** - a plain case-insensitive match over the same entries the Learning timeline already splits these files into, not embeddings; memory is kept small on purpose (the consolidation pass exists to keep it that way), so there's little a heavier semantic search would catch that this misses. Always-safe, and scoped to the calling agent's own memory only.
- **New `mix pepe browser install`: detects the host's Linux package manager and installs a real browser through it, for the `browser` agent tool to drive** - for a Linux install running outside Docker, where a downloaded browser can succeed at downloading and still fail to launch if the machine is missing shared libraries the official Docker image now ships by default. Prompts for `sudo` if it needs it, the same as if you'd typed the package manager invocation yourself; once installed, `browser` finds it on `PATH` directly and stops downloading anything on that machine. On macOS or Windows, where this is never actually needed (a downloaded browser already links against what those OSes always provide), it says so plainly instead of showing a Linux-shaped "couldn't detect a package manager" message that would read like something's broken.
- **New `browser` tool: an agent can drive a real, headless Chrome for pages `fetch_url` can't handle** - JavaScript-rendered content, a login, a multi-step flow. One session per conversation (cookies and the current page persist call to call until closed or idle for 10 minutes); interactive elements are addressed by a number from the last `open`/`snapshot`, not a CSS selector the model has to invent. Enforces the same http(s)-only, no-internal-address rule `fetch_url` already does, and is gated like `bash`, not always-safe. Looks for an already-installed Chrome/Chromium/Edge/Brave first (PATH and each OS's normal install locations), and if none is found, downloads a browser automatically and caches it - opt out with `PEPE_BROWSER_AUTO_DOWNLOAD=0`, or bake one into the Docker image instead with the `PEPE_IMAGE_APT_PACKAGES=chromium` build arg. The default Docker image now also ships the shared libraries a downloaded browser needs to actually launch, on both architectures the official image publishes - `browser` works out of the box on `amd64` and `arm64` alike (an Apple Silicon Mac or an ARM cloud host included), no build arg and no manual install either way. Linux on ARM downloads from Playwright's own CDN instead of Google's Chrome for Testing feed, since Google doesn't publish a build for that architecture at all - still a plain HTTPS download, no npm/Node.js involved.
- **New `session_search` tool: an agent can find and read past conversations, built entirely on the existing `traces` store** - a session's own live memory is gone the moment it ends or the app restarts, but its trace isn't. Actions: `list_sessions`, `search` (by prompt/tool-call content), `session_history` (one conversation's full timeline), `show` (one turn's complete transcript). Always-safe, scoped to the calling agent's own project.
- **New `mix pepe flow` command: promote a proven, repeated tool-call sequence (from real `traces`) into a script that replays without ever calling the model** - the third of the three features queued from the r/openclaw thread. Promotion requires 2+ traces that made the *exact* same ordered tool calls, refusing otherwise instead of guessing which parts of a call vary; a flow can be run on demand or scheduled (`mix pepe flow schedule`, a new cron kind that skips the model entirely). A flow only runs a step already covered by the agent's own `auto_approve`, since nobody is watching a scheduled flow fire to approve anything in the moment.
- **`mix pepe agent prompt NAME`, and a matching "Assembled prompt" section on the dashboard's agent edit page, print the fully-assembled system prompt** - not just the `system_prompt` field, everything Pepe actually sends as the system message (persona/identity/boot files, the behavior contract, the current time, the docs/skills index). Also fixed the `/v1/chat/completions` API to build this same assembled prompt when a caller sends no system message of its own, instead of the bare field alone - the same agent now gets the same prompt on every surface.
...
- **Watches move to the same SQLite store next** - the same treatment commitments got, for the same reason. Agent- and project-rename rebinding, and cleanup when an agent is deleted, all still work the same way.
- **Traces move to the same SQLite store too**, replacing one JSON file per run under `data/traces/<project>/<id>.json`. The dashboard's Traces page and `mix pepe traces` list recent runs noticeably faster, since that no longer means listing and decoding every file in a project's directory.
- **Boards and their cards move to the same SQLite store**, the last of `config.json`'s growing sections to make the move (usage and its own log come next). A card's `board` is now a real foreign key, so deleting a board deletes its cards with it at the database level instead of a hand-rolled filter.
- **Usage (the token/billing ledger) and the monthly customer-message counter move to the same SQLite store**, the last of the growing operational data that used to live as one JSONL file per project per month. `Pepe.Usage.summary/3`, `invoice/2`, budget/message-limit checks and the dashboard's usage pages are unchanged in behavior; `month_to_date/1` and `invoice/2` now run a real bounded query instead of reading a scope's whole file history. Renaming a project now also carries its usage, message-count and trace history along with it (a gap this closes for traces too, missed when traces moved).

### Changed
- **`mix pepe update` now points at `CHANGELOG.md` and at `mix pepe agent prompt NAME` right after a successful update** - a real release can change the hardcoded scaffolding wrapped around every agent's system prompt, with nothing in `config.json` to diff and no confirmation step, so the update itself now says where to look instead of leaving that silent.
...
- **Board card operations (claim, complete, block, and the rest of the transition set) and board deletion each had a narrow race window**: a card's crash-cleanup could wrongly fire against a card that had already been legitimately reclaimed and re-dispatched by someone else, a caller could receive a `nil` card if its board was force-deleted in the same instant, and deleting an empty board could cascade away a card created in the exact same moment even without `--force`. Each check and its write now happen as one atomic step.
- **Deleting an agent left behind any commitment or watch that was bound to it only by a raw, never-resolved handle** (an already-orphaned reference), instead of removing it along with the rest of that agent's bindings.
- **A trace that failed to save (a database hiccup) was silently swallowed with no record anywhere**: now logged as a warning, the same tolerance the config journal already has for the same kind of hiccup.
