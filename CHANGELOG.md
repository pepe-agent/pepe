...
- **Watches move to the same SQLite store next** - the same treatment commitments got, for the same reason. Agent- and project-rename rebinding, and cleanup when an agent is deleted, all still work the same way. A new `mix pepe config migrate-data` command (separate from `migrate-commitments`, which already shipped standalone) is the operator-run, safe-to-repeat import for everything still in `config.json` from before this - watches today, more sections folding in as the rest of this migration lands. `mix pepe doctor` flags a config.json that still carries an unmigrated `"watches"` section.
- **Traces move to the same SQLite store too**, replacing one JSON file per run under `data/traces/<project>/<id>.json`. The dashboard's Traces page and `mix pepe traces` list recent runs noticeably faster, since that no longer means listing and decoding every file in a project's directory. `mix pepe config migrate-data` picks up any traces left from before this - unlike watches/commitments, the old files are never deleted by the import (a diagnostic trail is safer to leave stale than to risk losing), so remove `data/traces/` by hand once you've confirmed the import.
- **Boards and their cards move to the same SQLite store**, the last of `config.json`'s growing sections to make the move (usage and its own log come next). A card's `board` is now a real foreign key, so deleting a board deletes its cards with it at the database level instead of a hand-rolled filter. `mix pepe config migrate-data` picks up any boards/cards left from before this; `mix pepe doctor` flags a config.json that still carries either section.
- **Usage (the token/billing ledger) and the monthly customer-message counter move to the same SQLite store**, the last of the growing operational data that used to live as one JSONL file per project per month. `Pepe.Usage.summary/3`, `invoice/2`, budget/message-limit checks and the dashboard's usage pages are unchanged in behavior; `month_to_date/1` and `invoice/2` now run a real bounded query instead of reading a scope's whole file history. Renaming a project now also carries its usage, message-count and trace history along with it (a gap this closes for traces too, missed when traces moved). `mix pepe config migrate-data` picks up any usage/message data left from before this - like traces, the source files are never deleted, since a billing audit trail is safer left stale than risked on a bug in an importer.
...
- **`mix pepe config migrate-data`'s board-card import could crash entirely on one orphaned card** (referencing a board id that doesn't exist, whether from a hand-edited `config.json` or the board's own import failing): now reported as a normal per-entry failure, same as any other malformed entry, instead of taking every other board/card down with it.
...
- **`mix pepe config migrate-data`'s legacy config-journal and usage/message-ledger imports could each report "already migrated" and refuse to run on almost any real install**, even one that had never actually imported anything: any ordinary config write or chat turn (or another subsystem's own migration finishing in the same run) lands a new row in that same table, which the import used to read as its own completion signal. Each now tracks completion by its own legacy file or directory's presence instead.
