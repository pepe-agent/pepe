...
- **`mix pepe config migrate-data`'s legacy config-journal import could report "already migrated" and refuse to run on almost any real install**, even one that had never actually imported anything: any ordinary config write (or another subsystem's own migration finishing in the same run) lands a new row in that same table, which the import used to read as its own completion signal. It now tracks completion by the source file's presence instead.
