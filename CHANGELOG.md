...
- **New `browser` tool: an agent can drive a real, headless Chrome for pages `fetch_url` can't handle** - JavaScript-rendered content, a login, a multi-step flow. One session per conversation (cookies and the current page persist call to call until closed or idle for 10 minutes); interactive elements are addressed by a number from the last `open`/`snapshot`, not a CSS selector the model has to invent. Enforces the same http(s)-only, no-internal-address rule `fetch_url` already does, and is gated like `bash`, not always-safe. Looks for an already-installed Chrome/Chromium/Edge/Brave first (PATH and each OS's normal install locations), and if none is found, downloads a small headless-only Chrome build automatically and caches it - opt out with `PEPE_BROWSER_AUTO_DOWNLOAD=0`, or bake one into the Docker image instead with the `PEPE_IMAGE_APT_PACKAGES=chromium` build arg. The default Docker image now also ships the shared libraries that downloaded build needs to actually launch (it's a built-in tool, not an optional extra like ffmpeg), so `browser` works out of the box on an `amd64` container with no build arg - `arm64`/Linux-ARM hosts get a clear "unsupported platform" error instead, since Google doesn't publish a Chrome for Testing build for that architecture.
