defmodule Cortex.WatchTest do
  ...
  setup do
    home = Path.join(System.tmp_dir!(), "cortex_watch_#{System.unique_integer([:positive])}")
    File.mkdir_p!(home)
  ...
  describe "due?/2" do
    test "a fresh pending watch is due; a future next_check is not; non-pending never is" do
      assert Cortex.Watch.due?(watch(next_check: nil), 1000)
      assert Cortex.Watch.due?(watch(next_check: 900), 1000)
      refute Cortex.Watch.due?(watch(next_check: 1100), 1000)
      refute Cortex.Watch.due?(watch(state: "paused", next_check: nil), 1000)
      refute Cortex.Watch.due?(watch(state: "done", next_check: nil), 1000)
    end
  end
  ...
  describe "evaluate/1 with a probe trigger" do
    ...
    test "a `contains` probe matches stdout regardless of exit code" do
      ...
      assert {%Watch{state: "done"}, "done"} = Cortex.Watch.evaluate(w)
    end
