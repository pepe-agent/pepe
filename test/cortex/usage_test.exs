defmodule Cortex.UsageTest do
  use ExUnit.Case, async: false

  alias Cortex.Config
  alias Cortex.Pricing
  alias Cortex.Usage

  setup do
    home = Path.join(System.tmp_dir!(), "cortex_usage_#{System.unique_integer([:positive])}")
    File.mkdir_p!(home)
  ...
  describe "invoice" do
    ...
    test "renders CSV and Markdown", %{month: month} do
      ...
      csv = Cortex.Usage.Invoice.to_csv(inv)
      ...
      md = Cortex.Usage.Invoice.to_markdown(inv)
      assert md =~ "# Invoice — acme"
      assert md =~ "markup **1.5**"
      assert md =~ "USD 11.25"
      assert Cortex.Usage.Invoice.basename(inv) == "acme-#{month}"
    end
    ...
    test "the export_invoice tool saves a file and returns it" do
      assert {:ok, out} = Cortex.Tools.Invoice.run(%{"company" => "acme"}, %{})
      assert out =~ "Saved invoice to"
      assert out =~ "# Invoice — acme"

      assert {:error, msg} = Cortex.Tools.Invoice.run(%{"company" => "ghost"}, %{})
