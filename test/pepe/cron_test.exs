defmodule Pepe.CronTest do
  ...
  setup do
    home = Path.join(System.tmp_dir!(), "pepe_cron_#{System.unique_integer([:positive])}")
    File.mkdir_p!(home)
  ...
  test "due? matches the scheduled minute in the cron's own timezone" do
    cron = sample()
    assert Pepe.Cron.due?(cron, ~N[2026-07-01 08:00:00])
    refute Pepe.Cron.due?(cron, ~N[2026-07-01 09:00:00])
  end

  test "next_run is a DateTime in the cron's timezone" do
    dt = Pepe.Cron.next_run(sample())
    assert %DateTime{} = dt
  ...
  test "an invalid schedule never fires and yields no next run" do
    ...
    refute Pepe.Cron.due?(cron, ~N[2026-07-01 08:00:00])
    assert Pepe.Cron.next_run(cron) == nil
  end
  ...
  test "missed?/1 detects a just-missed slot once, anchored to last_run" do
    ...
    assert {true, _slot} = Pepe.Cron.missed?(cron)
  ...
  test "missed?/1 is false outside the grace window" do
    ...
    refute Pepe.Cron.missed?(cron)
  end

  test "deliver \"none\" is a no-op, unknown targets go to the log" do
    assert Pepe.Cron.deliver("none", "anything") == :ok
    assert Pepe.Cron.deliver("log", "goes to logger") == :ok
  end
end
