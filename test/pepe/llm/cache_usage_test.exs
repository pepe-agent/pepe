defmodule Pepe.LLM.CacheUsageTest do
  ...
  reports cache-read AND cache-write separately from `input_tokens` (so prompt_tokens = input +
  cache_read + cache_creation); the Responses API already folds cached into `input_tokens`.
  """
  ...
  defmodule AnthropicCache do
    ...
    def call(conn, _) do
      events = [
        ~s({"type":"message_start","message":{"usage":{"input_tokens":10,"cache_read_input_tokens":85,"cache_creation_input_tokens":5,"output_tokens":1}}}),
  ...
  test "anthropic: prompt_tokens is input + cache_read + cache_write, cached_tokens is the cache-read portion" do
    ...
    assert res.usage["cached_tokens"] == 85
