# Configuring Cortex — overview

How Cortex is configured, and how you (an agent) can change it safely. Read the
focused docs (`agents`, `channels`, `scheduled-tasks`, `mcp`, `permissions`) for
each area.

## Where config lives

All configuration is a single JSON file at `~/.cortex/config.json` (overridable with
`CORTEX_HOME`/`CORTEX_CONFIG`). There is **no database**. Top-level keys:
...
reference**, e.g. `"api_key": "${OPENAI_API_KEY}"`. Cortex interpolates it at read
...
## The tools you use to configure Cortex
