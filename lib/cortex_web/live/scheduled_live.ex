defmodule CortexWeb.ScheduledLive do
  ...
  def handle_event("cron_create", params, socket) do
    ...
    cond do
      ...
      match?({:error, _}, Cortex.Cron.parse(schedule)) ->
        {:error, msg} = Cortex.Cron.parse(schedule)
        {:noreply, put_flash(socket, :error, gettext("Invalid schedule: %{msg}", msg: msg))}

      true ->
        Config.put_cron(%Cortex.Config.Cron{
          id: new_cron_id(name),
          name: name,
