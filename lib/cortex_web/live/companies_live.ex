defmodule CortexWeb.CompaniesLive do
  @moduledoc """
  Companies section: create, edit and delete tenant scopes. Each company walls off
  its own agents, workspaces, models and automations from every other. Root is the
  default, non-company workspace. A company's name is its identity key (it prefixes
  every agent handle), so only its description is editable — not its name.
  """
              ...
              <p class={hlp()}>
                {if @editing.new?,
                  do:
                    ...
                    gettext(
                      "The name can't change — it keys every agent, workspace and binding in this company."
                    )}
              </p>
            </div>
            <div>
