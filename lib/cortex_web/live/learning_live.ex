defmodule CortexWeb.LearningLive do
  @moduledoc """
  ...
  def mount(params, _session, socket) do
    ...
    {:ok,
     assign(socket,
       page_title: "Cortex · Learning",
       scope: params["scope"] || "all",
       companies: Config.companies(),
       new_company: false,
       learn_agent: agent,
       learn_nodes: Cortex.Learning.timeline(agent),
       editing: nil
     )}
  end
  ...
  defp load_node("skill", title, _agent) do
    ...
    builtin = Path.join(Application.app_dir(:cortex, "priv/skills"), "#{title}.md")
