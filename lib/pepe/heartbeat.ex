defmodule Pepe.Heartbeat do
  ...
  *when* to call it - Pepe doesn't hardcode a scheduler here; each surface (e.g.
  ...
  def pulse(session_key) do
    case Pepe.Heartbeat.Cooldown.allow?(session_key) do
      :ok -> Pepe.Agent.Session.heartbeat(session_key)
  ...
  defp brief(agent_name) do
    path = Path.join(Pepe.Agent.Workspace.dir(agent_name), "HEARTBEAT.md")
