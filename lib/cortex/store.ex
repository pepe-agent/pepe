defmodule Cortex.Store do
  ...
  This is the *disposable* tier of Cortex's storage: configs (and souls) are the
  source of truth in `~/.cortex/config.json`; the store is regenerable cache. If a
  user backs up only their config and drops `~/.cortex/data/`, nothing essential is
  lost.

  Data is namespaced and may carry a TTL:

      Cortex.Store.put(:session, "telegram:42", history, ttl: 3600)
      Cortex.Store.get(:session, "telegram:42")   #=> history (or nil once expired)
  ...
  on first use, on the local node, under `<CORTEX_HOME>/data/mnesia`.
  """
  ...
  defp do_start do
    dir = Path.join([Cortex.Config.home(), "data", "mnesia"])
    File.mkdir_p!(dir)
