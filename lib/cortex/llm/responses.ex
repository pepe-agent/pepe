defmodule Cortex.LLM.Responses do
  @moduledoc """
  ...
  than Chat Completions, so this module translates on both ends:
    ...
    * inbound  — Cortex's OpenAI chat-format `messages`/`tools` → Responses
      `instructions` + `input` items + flat `tools`.
    * outbound — the SSE `response.*` event stream → the same
      `%{content, tool_calls, finish_reason, usage}` result `Cortex.LLM` returns,
  ...
  Dispatched from `Cortex.LLM` when `model.api == "openai-responses"`.
  """
  ...
  def stream_chat(%Model{} = model, messages, on_delta, opts \\ [])
    ...
    model = Cortex.OAuth.ensure_fresh(model)
    ...
    collector = fn {:data, data}, {req, resp} ->
      state = resp.private[:cortex] || init
      ...
      {:cont, {req, %{resp | private: Map.put(resp.private, :cortex, state)}}}
    end
    ...
    case Req.post(req) do
      {:ok, %{status: status, private: %{cortex: state}}} when status in 200..299 ->
        {:ok, finalize(state)}

      {:ok, %{status: status} = resp} when status in 200..299 ->
        {:ok, finalize(resp.private[:cortex] || init)}

      # Non-2xx: the body was streamed into the collector, so read it from there.
      {:ok, %{status: status, private: %{cortex: %{raw: raw}}}} ->
