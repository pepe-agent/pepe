defmodule Cortex.Agent.SessionSupervisor do
  ...
  @doc "All live session keys, registered in `Cortex.Agent.Registry`."
  def list do
    Registry.select(Cortex.Agent.Registry, [{{:"$1", :_, :_}, [], [:"$1"]}])
  end

  @doc "Terminate a session (and forget its permission grants + persisted file)."
  def terminate(key) do
    Cortex.Permissions.SessionStore.clear(key)
    if persist?(), do: Cortex.Agent.SessionPersistence.delete(key)
  ...
  def restore do
    if persist?() do
      for {key, agent_name} <- Cortex.Agent.SessionPersistence.all() do
