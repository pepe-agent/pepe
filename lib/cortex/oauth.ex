defmodule Cortex.OAuth do
  ...
  local loopback server (`Cortex.OAuth.Callback`), and exchanges the code for
  ...
  A *flow spec* is a plain map (declared per provider in `Cortex.Providers`):
  ...
  """
  def ensure_fresh(%Cortex.Config.Model{oauth: oauth} = model) when is_map(oauth) do
  ...
  defp persist_refresh(model, oauth, tokens) do
    ...
    Cortex.Config.put_model(updated)
    updated
  end
