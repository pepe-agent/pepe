defmodule Cortex.Doctor do
  ...
  defp agent_checks do
    known = MapSet.new(Cortex.Tools.names())
    ...
    Enum.flat_map(Config.agents(), fn agent ->
      ...
      unknown =
        agent.tools
        |> Enum.reject(&(MapSet.member?(known, &1) or Cortex.MCP.mcp_tool?(&1)))
  ...
  defp cron_checks do
    Enum.flat_map(Config.crons(), fn cron ->
      [
        case Cortex.Cron.parse(cron.schedule) do
  ...
  defp model_checks do
    Enum.map(Config.models(), fn model ->
      case Cortex.LLM.list_models(model) do
