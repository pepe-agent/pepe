defmodule Cortex.LLM do
  ...
  base URL, key, model and generation params come from an `Cortex.Config.Model`.
  ...
  def chat(%Model{api: "openai-responses"} = model, messages, opts),
    do: Cortex.LLM.Responses.chat(model, messages, opts)
  ...
  def stream_chat(%Model{api: "openai-responses"} = model, messages, on_delta, opts)
      ...
      do: Cortex.LLM.Responses.stream_chat(model, messages, on_delta, opts)
  ...
  def list_models(%Model{api: "openai-responses"} = model),
    do: Cortex.LLM.Responses.list_models(model)
