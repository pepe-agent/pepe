defmodule Cortex.Company do
  ...
  directory, session keys, the `Cortex.Agent.Registry`, `can_message` routes, cron
  ...
  `Cortex.Tools.SendToAgent`).

  The **root** scope (`company == nil`) is what every command operates on when no
  `--company` is given.
  """

  @separator "/"

  @doc "The handle separator (`\"/\"`), forbidden inside a company or agent name."
  def separator, do: @separator

  @doc "Build a handle from an optional company and a bare name."
  @spec handle(String.t() | nil, String.t()) :: String.t()
  def handle(company, name) when company in [nil, ""], do: to_string(name)
  def handle(company, name), do: "#{company}#{@separator}#{name}"

  @doc "Split a handle into `{company | nil, bare_name}`."
  ...
  def split(handle) when is_binary(handle) do
    case String.split(handle, @separator, parts: 2) do
      [company, name] when company != "" and name != "" -> {company, name}
      _ -> {nil, handle}
    end
  end

  @doc "The company a handle belongs to, or `nil` for the root scope."
  @spec of(String.t()) :: String.t() | nil
  def of(handle), do: handle |> to_string() |> split() |> elem(0)

  @doc "The bare (display) name of a handle, without any company prefix."
  @spec name_of(String.t()) :: String.t()
  def name_of(handle), do: handle |> to_string() |> split() |> elem(1)

  @doc "Are two handles in the same scope (same company, or both root)?"
  ...
  is qualified into the sender's company, an already-qualified handle is left as-is.
