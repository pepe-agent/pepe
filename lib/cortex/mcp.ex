defmodule Cortex.MCP do
  @moduledoc """
  Facade for MCP (Model Context Protocol) tool servers.

  Configured servers (`Cortex.Config.mcp_servers/0`) are launched **on demand** —
  one `Cortex.MCP.Client` per server, started lazily under a DynamicSupervisor and
  ...
  alias Cortex.Config
  alias Cortex.MCP.Client

  @registry Cortex.MCP.Registry
  @sup Cortex.MCP.DynSup
