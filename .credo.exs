    ...
    %{
      ...
      checks: %{
        enabled: [
          ...
          #
          ## ExSlop (AI-slop detection) - listed explicitly rather than relying on
          ## the plugin's auto-registered default config: Credo's config-merge order
          ## between a project's own `checks.enabled` and a plugin's registered
          ## `checks.extra` isn't reliable (verified empirically - a probe file with
          ## a textbook blanket-rescue + narrator-comment slipped through silently
          ## with only `plugins: [{ExSlop, []}]` set). This is ExSlop's own
          ## `recommended_checks/0` list (30 of its 40 checks - the noisier
          ## style/perf ones are opt-in, see ExSlop's README before adding them).
          #
