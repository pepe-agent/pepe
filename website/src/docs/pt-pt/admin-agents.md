...
| `set_flag` | Liga ou desliga um interruptor do agente alvo (`on`/`off`): `trust_untrusted_content` (deixar que ele atue sobre o que estranhos enviam), `exempt_message_limit`, `midrun_fold` (deixar uma correção a meio do turno direcionar o turno em curso em vez de esperar sempre; usa o `triage_model` se existir, senão o próprio modelo do agente — uma chamada extra por mensagem a meio do turno), `commitments` (reparar num seguimento declarado depois de cada turno e acompanhá-lo, sem que lhe seja pedido), ou `session_search_project_wide` (deixar o `session_search` ver todas as conversas do projeto do agente alvo, não apenas as do próprio chamador — vê [Busca de sessões](../session-search/); desligado é o correto para um agente que fala com vários clientes finais diferentes). Ligar o `trust_untrusted_content` ou o `session_search_project_wide` não pode ser feito a partir de uma execução que ela própria ingeriu conteúdo de fora, por isso um documento injetado não consegue virar nenhum dos dois. |
