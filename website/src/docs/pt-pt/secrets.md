...
Agora o agente pode correr `op` sozinho: `op vault list`, `op item get "Prod DB"`, e usar o que encontrar. O **skill `vaults`** incorporado ensina-lhe o fluxo inteiro, incluindo a regra que importa: preferir **`op run`** e **`op inject`**, que entregam o segredo a um comando ou a um template sem o valor alguma vez ser impresso, em vez de fazer `op read` dele à vista. O agente instala o `op` sozinho se faltar. E se o token existir mas ainda estiver removido do shell dele, o próprio agente acrescenta o nome ao `expose_env` através da ferramenta `config_set`, protegida por barreira de permissão (uma lista de nomes, nunca um valor), em vez de esperar que sejas tu a abrir a porteira.

<div class="note"><strong>Isto troca uma fronteira por fluidez, de propósito.</strong> Um token de conta de serviço do 1Password só abre os cofres para os quais o delimitaste, por isso o raio de dano é exatamente esse âmbito. O Pepe ainda limpa o valor exato de todo o segredo que conhece da saída de ferramenta, e mascara qualquer coisa com *cara* de credencial que não conhece (<code>PGPASSWORD=…</code>, <code>Bearer …</code>, um JWT), antes de chegar ao modelo ou ao trace. Por isso um <code>env</code> à toa, um erro verboso, e até um valor que o agente leia com <code>op read</code> são apanhados. O que resta é só um segredo que o Pepe nem conhece nem tem cara de segredo; o skill encaminha para o <code>op run</code>, e o âmbito do token limita o resto. Usa um token estreitamente delimitado, ou não ligues isto.</div>
