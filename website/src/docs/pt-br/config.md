...
Tudo vive dentro de `~/.pepe/` (ou de `PEPE_HOME`). Não há servidor de banco de dados: o `config.json` é a única fonte da verdade para projetos, agentes, modelos, watches, crons, bots, servidores MCP e tokens de API já com hash. Projetos e agentes carregam um id interno estável, então renomear um deles só troca o rótulo e move o diretório, sem quebrar nenhuma referência que aponte para esse id. O conhecimento de um agente vive como arquivos em `projects/<slug>/agents/<name>/` (o projeto padrão entra aí também, com o próprio slug), o histórico das conversas fica em `data/sessions/`, e `data/mnesia/` é um cache descartável que se reconstrói sozinho. O `Pepe.Repo` e o Postgres existem no código, mas estão desligados (`ecto_repos: []`); são a porta deixada aberta para um futuro backend de banco de dados, hoje sem uso.
