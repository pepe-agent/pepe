...
<div class="note"><strong>Isto troca uma fronteira por fluidez, de propósito.</strong> Um token de conta de serviço do 1Password só abre os cofres para os quais você o escopou, então o raio de dano é exatamente esse escopo. O Pepe ainda raspa o valor exato de todo segredo que ele conhece da saída de ferramenta, e mascara qualquer coisa com *cara* de credencial que ele não conhece (<code>PGPASSWORD=…</code>, <code>Bearer …</code>, um JWT), antes de ir pro modelo ou pro trace. Então um <code>env</code> à toa, um erro verboso, e até um valor que o agente leia com <code>op read</code> são pegos. O que sobra é só um segredo que o Pepe nem conhece nem tem cara de segredo; o skill empurra pro <code>op run</code>, e o escopo do token limita o resto. Use um token estreitamente escopado, ou não ligue isto.</div>
