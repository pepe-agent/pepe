...
description: Arquive a instalação inteira, ou retire uma empresa para rodar no próprio servidor, e restaure qualquer uma das duas com um único comando.
---
...
Este é o arquivo do tipo "não perca esta máquina". Ele empacota todas as empresas, todos os workspaces dos agentes, o espaço compartilhado, as sessões e os livros-razão de uso, e pula `data/mnesia/` (um cache descartável que se reconstrói sozinho). Restaurado em uma máquina vazia, ele é a mesma máquina de novo.

## Extração: uma empresa, por conta própria
...
Uma empresa que cresceu dentro de uma instalação compartilhada pode sair para rodar no próprio servidor. Você não chega lá copiando uma pasta, porque as entradas dessa empresa estão entrelaçadas no `config.json` compartilhado como identificadores `acme/agente`. A extração reescreve esses identificadores para nomes de raiz simples, então o arquivo é uma **instalação nova e de um único inquilino que por acaso é aquela empresa** — coloque em um servidor novo e execute.

Só aquela empresa viaja: seus agentes, modelos, crons, watches, bots, tokens, workspaces e histórico de uso. Nada dos outros inquilinos vai junto. Se um dos seus agentes depende de um **modelo compartilhado** (um que vive na raiz, não dentro da empresa), esse modelo também é puxado para o arquivo, para o pacote funcionar em uma máquina vazia; o comando informa quais.
