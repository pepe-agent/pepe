...
volta pro embutido naquela chamada específica e registra isso. Um ocupante de plugin se
...
| `compaction` | Resume o meio de uma conversa longa com o próprio modelo | Como uma conversa longa é condensada pra caber na janela de contexto |
...
## Escopando um slot pra um agente ou projeto

O `pepe slot set` acima fixa um slot pra instalação inteira - todo agente recebe o mesmo
ocupante. Um agente pode sobrescrever isso pra si mesmo:
...
Um projeto pode ter seu próprio padrão pra todo agente nele, no mesmo formato que o
...
embutido ignora `:mode`; um backend mais rico (um vector store) é livre pra usá-la.
`index/1` é opcional, pra um backend que mantém o próprio armazenamento e precisa
reconstruí-lo.
...
Pepe") - vale pra qualquer ocupante que responda. Esse é o único slot onde o próprio
`timeout_ms` por chamada do `bash` (não o teto generoso de 5 minutos do slot) é o prazo
real pro embutido; um ocupante de plugin ainda deve responder rápido, já que o teto do
slot é uma rede de segurança, não um orçamento pra gastar.
...
Um uso natural: trocar pra um modelo mais barato quando o gasto de um projeto se aproxima
...
Um plugin aqui decide como uma conversa longa é condensada pra caber na janela de contexto
...
falha de vez; uma quebra/timeout ainda degrada pro embutido naquela chamada).
...
3. Aponte o slot `compaction` pra ele - pra instalação inteira, ou só pra um agente/projeto:
...
4. Confirme que está ativo: `pepe slot list` mostra o ocupante; uma quebra ou timeout cai
   de volta pro embutido naquela chamada, e isso também fica visível ali.
...
um bem-comportado chama `opts[:on_event]` por conta própria pra que uma superfície de chat
ao vivo continue transmitindo, e pode chamar `Pepe.Trace.event/1`/
`Pepe.Agent.RunObservers.notify/1` pra ter fidelidade total de trace/observador se quiser.
...
1. Escreva um módulo implementando `name/0`, `slot/0` (devolvendo `"harness"`), e `run/3`.
   ...
   resposta inteira - o menor harness real possível, no lugar de "delegar pra uma CLI de
   agente diferente":
...
3. Fixe o slot `harness` nele - essa é uma decisão maior que a maioria dos slots, já que
   entrega ao plugin o turno inteiro (veja acima), então escopar pra um agente só enquanto
...
- **Um canal de chat de conexão persistente** (um plugin implementando
  `Pepe.Gateways.Channel` - pra uma plataforma como Discord ou Matrix que precisa de um
  websocket de longa duração, não só um webhook de entrada) roda junto de todo outro canal,
  incluindo o Telegram, no próprio domínio de falha supervisionado, pra um que se comporte
  mal não conseguir derrubar os outros junto. Veja [Plugins](/docs/plugins) pro formato
  baseado em webhook `Pepe.Webhooks.Provider`, que a maioria dos plugins de canal deveria
  buscar primeiro - um canal persistente é pras plataformas que um webhook genuinamente não
  cobre.
