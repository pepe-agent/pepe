...
description: Peça a um agente para fazer algo por você em um horário fixo, sem ninguém no teclado. Um resumo matinal, uma checagem de hora em hora, entregue onde você quiser.
---

## Tarefas recorrentes

Uma tarefa agendada é um trabalho que você entrega uma vez a um agente e que passa a acontecer sozinho: "todo dia útil às 9, resuma os erros da noite e me mande". Por baixo, uma tarefa é um prompt autossuficiente, um horário, um fuso horário e um lugar para entregar o resultado. Quando dispara, o Pepe roda o agente sobre esse prompt em uma **sessão nova, sem histórico de chat**. Nada de nenhuma conversa anterior é carregado, então o prompt precisa dizer tudo o que a execução precisa (o que fazer, quais dados olhar, a janela de tempo).
...
Rode `pepe serve` e abra a página **Scheduled**. Ela lista cada tarefa com o próximo horário de execução e te dá as mesmas ações como botões: criar uma tarefa nova com um formulário, forçar uma execução agora, ativar ou desativar, editar, remover e abrir o histórico de uma tarefa ali mesmo. O formulário de criação cobre tudo o que a CLI faz: o agente, o prompt, o horário, o fuso horário, o modelo e para onde entregar o resultado, incluindo a opção "Não enviar a lugar nenhum". Quando você digita o horário de uma tarefa, o painel consegue transformar uma frase simples como "todo dia útil às 9:30" na expressão cron correspondente para você, usando um modelo configurado, e valida o resultado antes de salvar.
...
O cronômetro vive dentro do processo da aplicação, então só roda enquanto o `pepe serve` ou o `pepe gateway` está de pé, e nunca durante um comando de execução única. Cada tarefa cuja hora chegou roda no próprio processo, então várias tarefas que caem no mesmo minuto disparam ao mesmo tempo, e uma tarefa lenta nunca bloqueia outra. As definições das tarefas ficam guardadas em `~/.pepe/config.json`, sob `"crons"`.
...
Uma tarefa cuja execução anterior **ainda está rodando** quando chega o próximo horário é **pulada**, e o pulo é escrito no histórico dela.

Ela é pulada em vez de acumulada porque uma tarefa aqui não é um script idempotente, é **um turno de agente**. Custa uma chamada ao modelo, tem efeito colateral (uma mensagem entregue, um arquivo escrito), e toda execução da mesma tarefa compartilha um único workspace do agente. Um job que leva sete minutos num agendamento de cinco iria acumular: duas execuções, depois três, depois quatro, cada uma cobrada, o relatório entregue duas vezes, e duas execuções escrevendo por cima uma da outra. Você descobriria pela fatura.

E ela nunca é pulada em silêncio. O pulo vira uma entrada de falha no histórico, e ela diz o que está errado:

> ⏭️ skipped: the previous run was still going. This job takes longer than its own schedule allows.

Essa entrada é o ponto. Sem ela, o job simplesmente pararia de acontecer, no horário certo, e o primeiro sinal seria que aquilo que ele fazia deixou de ser feito.

```bash
pepe cron add --name "digest" --prompt "..." --schedule "*/5 * * * *" --overlap
```

O `--overlap` (ou `"overlap": true` na configuração) roda mesmo assim, para a tarefa em que a concorrência é realmente o que você quer.
...
1. **A ferramenta precisa estar na lista de permissões do agente.** Um agente novo já nasce com `schedule_task` habilitada, como qualquer outra ferramenta; remova-a da lista do agente se ele nunca deve poder agendar nada.
...
Assim uma tarefa nunca aparece pelas suas costas: o recurso pode ser removido por agente, e cada tarefa concreta ainda depende da sua autorização por cima disso.
