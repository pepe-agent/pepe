...
| `set_flag` | Liga ou desliga um interruptor do agente alvo (`on`/`off`): `trust_untrusted_content` (deixar que ele aja sobre o que estranhos mandam), `exempt_message_limit`, `midrun_fold` (deixar uma correção no meio do turno direcionar o turno em andamento em vez de sempre esperar; usa o `triage_model` se houver um, senão o próprio modelo do agente, ao custo de uma chamada a mais por mensagem no meio do turno), `commitments` (perceber um follow-up declarado depois de cada turno e acompanhá-lo, sem que seja pedido), ou `session_search_project_wide` (deixar o `session_search` ver todas as conversas do projeto do agente alvo, não só as do próprio chamador; veja [Busca de sessões](../session-search/). Desligado é o certo para um agente que fala com vários clientes finais diferentes). Ligar o `trust_untrusted_content` ou o `session_search_project_wide` não pode ser feito a partir de uma execução que ela mesma ingeriu conteúdo de fora, então um documento injetado não consegue virar nenhum dos dois. |
