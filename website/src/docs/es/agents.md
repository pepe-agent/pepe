...
Quien ya usa un agente para una sola cosa a menudo nunca descubre que también puede vigilar algo por cambios, ejecutar una tarea recurrente, o trabajar hacia un objetivo hasta que de verdad esté cumplido —nada muestra eso más allá de lo que la propia persona del agente llegue a mencionar. `capability_nudge`, desactivado por defecto, deja que el agente añada una frase corta y natural señalando una funcionalidad relacionada justo después de ayudar con algo, cuando de verdad encaja —no es un menú, ni en cada turno. Descubrimiento por el uso, no una avalancha de onboarding.

```bash
pepe agent add support --capability-nudge ...
```

Déjalo desactivado en un agente que deba mantenerse breve y transaccional; actívalo desde el editor de agentes del panel, o pidiéndole a un agente con la tool `manage_agent` que active el interruptor `capability_nudge` en otro agente.
...
- Permitir durante el resto de esta ejecución. Solo se ofrece mientras la ejecución
  ha ingerido contenido de fuera (ver [Seguridad y entorno aislado](../security/)):
  el único tipo de pre-aprobación que sigue funcionando realmente durante esa ventana.
...
# Imprime el system prompt completamente ensamblado - no solo el campo de persona,
# todo lo que Pepe construye alrededor. Ver "Ver exactamente lo que ve el modelo" abajo.
...
Define `langfuse_prompt` con el nombre de un prompt y la persona de este
agente pasa a venir de [Langfuse](../langfuse/) en lugar de su propio
`system_prompt`/`SOUL.md`: edita el prompt en Langfuse y el cambio llega a
Pepe en pocos minutos, sin redeploy y sin tocar `config.json`.

```bash
pepe agent add support --langfuse-prompt support-persona
```

Opt-in por agente: un agente sin `langfuse_prompt` definido queda totalmente
sin cambios, y uno cuya obtención falle (inaccesible, el nombre no resuelve)
cae directo a la persona local. Configuración y credenciales:
[Langfuse](../langfuse/).

## Ver exactamente lo que ve el modelo

El campo `system_prompt` es solo la semilla. Lo que realmente llega al modelo como
mensaje de sistema también incluye los archivos de persona/identidad/arranque del
agente, si los tiene, un contrato de comportamiento breve, la hora actual y un índice
de los docs y skills que conoce - nada de esto aparece si solo lees el campo en
disco. Para ver todo, ensamblado exactamente como lo enviaría una conversación real:

```bash
pepe agent prompt NAME
```

La página de edición del agente en el dashboard tiene la misma vista, bajo
**Prompt ensamblado** - contraída por defecto, ya que puede ser larga.
