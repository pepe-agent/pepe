...
| `data-logo` | Una imagen cuadrada pequeña, usada como icono de la burbuja y junto al título de la cabecera. Omítelo para mantener la burbuja con el emoji simple. | ninguno |
| `data-color` | Color de acento para la burbuja, la cabecera y los botones. | `#ea580c` |
| `data-theme` | `dark` o `light` - los colores base del panel bajo la cabecera. | `dark` |
...
El botón 🧹 de la cabecera empieza una conversación nueva de inmediato: cierra
la conexión actual, limpia el panel y se reconecta con un id de sesión nuevo.
Ese id se guarda al instante, así que incluso recargar la página por completo
sigue hablando con la conversación nueva, no con la anterior.
