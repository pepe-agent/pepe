...
| `set_flag` | Activa o desactiva un interruptor del agente objetivo (`on`/`off`): `trust_untrusted_content` (dejar que actúe sobre lo que los desconocidos envían), `exempt_message_limit`, `midrun_fold` (dejar que una corrección a mitad del turno dirija el turno en curso en vez de esperar siempre; usa el `triage_model` si existe, si no el propio modelo del agente — una llamada extra por mensaje a mitad del turno), `commitments` (detectar un seguimiento declarado después de cada turno y hacerle seguimiento, sin que se lo pidan), o `session_search_project_wide` (dejar que `session_search` vea todas las conversaciones del proyecto del agente objetivo, no solo las del que llama — ver [Búsqueda de sesiones](../session-search/); desactivado es lo correcto para un agente que habla con varios clientes finales distintos). Activar `trust_untrusted_content` o `session_search_project_wide` no puede hacerse desde una ejecución que ella misma ha ingerido contenido de fuera, así que un documento inyectado no puede activar ninguno de los dos. |
