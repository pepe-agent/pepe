...
Un navegador bajo el control de un agente llega a la misma red que la aplicación, así que `browser` aplica la misma regla que `fetch_url`: solo `http`/`https`, y nunca una dirección interna o privada (loopback, RFC1918, link-local, metadatos de la nube). Esa comprobación no se limita a la URL que le pasas a `open` - un enlace al que la propia página enlaza, una redirección de JavaScript, el envío de un formulario, o las propias peticiones de fondo de la página se comprueban de la misma forma, y se bloquean antes de que Chrome llegue siquiera a enviarlas, no solo la dirección que tú escribiste. Y porque un navegador real es una superficie bastante mayor que una herramienta de solo lectura (los scripts propios de la página se ejecutan, una sesión iniciada podría quedar expuesta, usa CPU y memoria reales), `browser` no es siempre-segura: cada llamada pasa por el mismo aviso de permiso que `bash`.
...
La imagen por defecto no incluye el paquete del navegador en sí (la misma lógica que mantiene el ffmpeg fuera - ver el Dockerfile), pero sí incluye las bibliotecas compartidas que un navegador descargado necesita para arrancar, en las dos arquitecturas que la imagen oficial publica (`amd64` y `arm64`), ya que `browser` es una herramienta integrada, no un extra opcional. Así que el paso 3 es lo que corre por defecto en Docker, y funciona de entrada: sin build arg, sin instalación manual, en cualquiera de las dos arquitecturas - incluyendo un Mac con Apple Silicon o un host en la nube ARM, no solo `amd64`. Si prefieres incluir un navegador completo en la imagen en vez de descargarlo en tiempo de ejecución:
...
Un navegador descargado necesita bibliotecas compartidas que el sistema base ya debe tener - fuera de Docker eso no está garantizado como en la imagen oficial: una distro de escritorio normalmente ya las tiene (otras apps con interfaz gráfica dependen de las mismas bibliotecas), pero un servidor mínimo o headless puede que no. Si `browser` descarga con éxito pero falla al arrancar, ejecuta:

```
mix pepe browser install
```

Detecta tu gestor de paquetes (`apt`/`dnf`/`yum`/`pacman`/`apk`/`zypper`) e instala un navegador completo a través de él, pidiendo tu contraseña de `sudo` si la necesita - ya pediste exactamente esto al ejecutar el comando. Una vez instalado, `browser` lo encuentra directamente en el `PATH` y deja de descargar nada.

## Linux en ARM

Google no publica un build de Chrome for Testing para Linux en ARM, así que ahí el paso 3 usa el propio CDN de Playwright en su lugar (una descarga HTTPS normal, igual que Chrome for Testing - sin npm ni Node.js de por medio) - la única diferencia es que descarga el Chromium completo en vez del build más pequeño sin interfaz, ya que ese CDN no ofrece un build sin interfaz como artefacto propio. De cualquier forma, esto es automático: un host ARM no necesita ninguna configuración especial, ni en Docker ni fuera de él.
