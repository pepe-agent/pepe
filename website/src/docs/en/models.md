---
title: Models
description: Connect Pepe to any OpenAI-compatible model provider, pick a default, and add fallbacks that take over when a provider has a bad moment.
---

## 3. Connect a model

Pepe works with any provider that speaks the OpenAI protocol. Give it your key
as the name of an environment variable, so the key itself is never written into
the config file.
