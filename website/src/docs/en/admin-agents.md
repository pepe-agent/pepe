...
| `set_flag` | Turn one of the target's switches on or off (`on`/`off`): `trust_untrusted_content` (let it act on content strangers send it), `exempt_message_limit`, `midrun_fold` (let a mid-turn correction steer the running turn instead of always waiting; uses `triage_model` if set, otherwise the target's own model — an extra call per mid-turn message), `commitments` (notice a stated follow-up after each turn and track it, without being asked), or `session_search_project_wide` (let `session_search` see every conversation in the target's project, not just the caller's own - see [Session search](../session-search/); off is right for an agent that talks to several different end customers). Turning `trust_untrusted_content` or `session_search_project_wide` on cannot be done from a run that has itself taken in outside content, so an injected document cannot flip either. |
