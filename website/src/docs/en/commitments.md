...
<div class="note"><strong>No database, no separate store.</strong> Commitments are plain records in <code>~/.pepe/config.json</code> (under <code>"commitments"</code>), fired by the same kind of in-process timer that drives watches and scheduled tasks - it only ticks while a long-lived surface (<code>pepe serve</code>, a gateway, or an interactive session) is up.</div>
