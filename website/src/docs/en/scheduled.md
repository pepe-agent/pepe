...
description: Have an agent do something for you on a schedule, with nobody at the keyboard. A morning brief, an hourly check, delivered wherever you want.
---

## Recurring tasks

A scheduled task is a job you hand to an agent once and it happens on its own from then on: "every weekday at 9, summarize last night's errors and send them to me". Under the hood, a task is a self-contained prompt, a schedule, a timezone, and a place to deliver the result. When it fires, Pepe runs the agent on that prompt in a **fresh session with no chat history**. Nothing from any earlier conversation is carried in, so the prompt has to say everything the run needs (what to do, which data to look at, the time window).
