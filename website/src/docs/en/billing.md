...
Every model call is metered and attributed to the agent's project, so you can bill a client per token. Metering happens at the single point every surface flows through (the console, the HTTP `/v1` API, the WebSocket, Telegram, and every webhook channel), and it appends to a durable, append-only ledger at `~/.pepe/data/usage/<slug>/YYYY-MM.jsonl` (where `<slug>` is the project's slug, e.g. `default`). That file is the audit trail for what gets charged.
