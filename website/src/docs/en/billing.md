...
All three are optional and independent - set any of them, all of them, or none. Root (the default, non-company scope) can carry the same caps, set with `pepe company set root ...` - it isn't a real company (it never shows in `company list`, can't be renamed or removed), but it isn't excluded from billing limits either.
...
An individual agent can be exempted from the message cap entirely - useful for something like an always-on escalation agent that must never go quiet because the rest of the company hit its cap:
