...
- `session_any` ("Allow for this session"): a blank cheque for the rest of this conversation - every future call to this tool runs without asking, whatever risks it carries, not just the ones this particular call happened to flag. Kept in memory, forgotten when you start a new session or restart. Other sessions still ask. (A narrower version scoped to calls shaped like this one also exists internally, but isn't offered as its own button - the two read as the same choice to a person deciding in the moment.)
- `session_bypass` (⚠️ "Allow everything for this session"): the broadest session grant there is - every tool, every risk, for the rest of this session. Unlike every other option here, it is not suspended even when the run has read something from a stranger (see below) - use it only for a session you already trust completely.
