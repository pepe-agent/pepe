...
You can run it while Pepe is up. The database (commitments, watches, traces, boards, usage) is never copied while it might be mid-write: instead `backup` takes a snapshot through the live connection, guaranteed consistent as of one moment in time (transactionally consistent), and verifies it before it goes in the archive. The command aborts rather than ship a snapshot that failed the check. Re-check an archive you already have with:
